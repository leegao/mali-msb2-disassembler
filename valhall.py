...
def build_imm(el):
    return Immediate(
        ...
        int(el.attrib["start"]),
        int(el.attrib["size"]),
        bool(el.attrib.get("signed", False)),
    )
...
def build_instr(el, overrides={}):
    ...
    # Get implicit ones
    sources = sources + (
        [Source(i, int(tsize)) for i in range(int(el.attrib.get("srcs", 0)))]
    )
...
def valhall_parse_isa(xmlfile):
    ...
    MODIFIERS = {
        ...
        "skip": Flag("skip", 39),
        "register_width": Modifier(
            "register_width", 46, 1, force_enum="register_width"
        ),
        "secondary_register_width": Modifier(
            "secondary_register_width", 47, 1, force_enum="register_width"
        ),
